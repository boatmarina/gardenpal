...
"""
        ...
        if request.method == "POST":
            ...
            form_values = {
                "name": request.form.get("name", "").strip() or (
                    request.form.get("label_plant_name", "").strip()
                    if request.form.get("active_mode") == "label" else ""
                ),
    ...
    def yard_plant_new():
        ...
        if request.method == "POST":
            ...
            form_values = {
                "zone_id": request.form.get("zone_id", "").strip(),
                "plant_name": request.form.get("plant_name", "").strip() or (
                    request.form.get("label_plant_name", "").strip()
                    if request.form.get("yard_input_mode") == "label" else ""
                ),
                "scientific_name": request.form.get("scientific_name", "").strip(),
                "lookup_query": request.form.get("lookup_query", "").strip(),
    ...
    def export_diary_pdf():
        try:
            ...
            return "PDF generation unavailable (xhtml2pdf not installed)", 500
        ...
        if ornamentals:
            ...
            for row in db.execute(
                f"""SELECT pt.plant_id, t.name, t.color
                    FROM plant_tags pt JOIN tags t ON t.id = pt.tag_id
                    WHERE pt.plant_id IN {oid_ph} ORDER BY t.name ASC""",
                oids,
            ).fetchall():
                tags_map.setdefault(row["plant_id"], []).append({"name": row["name"], "color": row["color"]})
            for row in db.execute(
        ...
        buf = _io.BytesIO()
        result = pisa.CreatePDF(_io.StringIO(html), dest=buf)
        if result.err:
            ...
            for _ in range(10):
                ...
                for block in response.content:
                    ...
                    try:
                        ...
                        elif block.name == "set_watering_tracking":
                            eid = inp.get("entry_id")
                            track = inp.get("track_watering")
                            if eid is None or track is None:
                                result = {"error": "entry_id and track_watering are required"}
                            else:
                                entry = db.execute(
                                    "SELECT id, user_id FROM garden_entries WHERE id = ?", (eid,)
                                ).fetchone()
        ...
        try:
            ...
            try:
                ...
                if not isinstance(recent_suggestions, list):
                    recent_suggestions = []
                # Normalise: old code may have stored full dicts instead of name strings
                recent_suggestions = [
            ...
            try:
                ...
                if not isinstance(queue, list):
                    queue = []
                # Filter out any malformed queue items
                queue = [q for q in queue if isinstance(q, dict) and q.get("name")]
            except Exception:
                queue = []

            if queue:
                # Pop the next queued suggestion and fetch its photo
                raw = queue.pop(0)
                db.execute(
                ...
                # Queue empty — generate one suggestion (count=1 keeps response ~200 tokens,
                # comfortably within the 5s API timeout; count=5 generated ~1000 tokens and
                # always timed out, leaving the queue permanently empty)
    ...
    try:
        ...
        resp = client.messages.create(
            ...
            system=(
                ...
                "\n"
                "Plant age: Always use the most recent planting or transplanting date, not the original. "
                "Seedlings under 3-4 weeks from direct sow should not be fertilized until true leaves appear; "
                "nursery starts are past this stage and can be fertilized soon after transplanting. "
                "Planting notes may contain important context — read them.\n"
                "\n"
                "NOT_NEEDED: Only when fertilizing would be actively harmful or provide zero benefit — "
                "plant is dead/dying, annual has fully senesced, or fertilizer directly causes harm "
                "(e.g. excess nitrogen forks root vegetables). "
                "'Days to maturity' does NOT mean the plant is done — fruiting crops continue producing "
                "and need ongoing fertilization. Never use NOT_NEEDED for a plant that is actively growing or fruiting.\n"
                "\n"
                "Line 1 rules — read carefully:\n"
                "- Overdue (today is past last_fertilized + typical interval): return today's date.\n"
                "- Just fertilized today: return a future date (never today — that re-triggers immediately).\n"
                "- Should not fertilize: return NOT_NEEDED, never a date. A date always means 'fertilize on this day'.\n"
                "\n"
                "Reply with ONLY:\n"
                "Line 1: YYYY-MM-DD (today if overdue, a future date otherwise) OR NOT_NEEDED\n"
    ...
    try:
        ...
        resp = client.messages.create(
            ...
            system=(
                ...
                "\n"
                "NOT_NEEDED: Only when fertilizing would be actively harmful or provide no meaningful benefit — "
                "plant is dormant, dead, or fertilizer would directly cause harm.\n"
                "\n"
                "Line 1 rules:\n"
                "- Overdue (today is past last_fertilized + typical interval): return today's date.\n"
                "- Just fertilized today: return a future date (never today).\n"
                "- Should not fertilize: return NOT_NEEDED, never a date.\n"
                "\n"
                "Reply with ONLY:\n"
                "Line 1: YYYY-MM-DD (today if overdue, a future date otherwise) OR NOT_NEEDED\n"
